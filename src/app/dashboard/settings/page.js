...
function SectionTitle({ children, dark }) {
  return (
    <h2 className={`text-base font-bold mb-4 font-jakarta ${dark ? "text-white" : "text-slate-900"}`}>
      {children}
    </h2>
  );
}

function InputField({ label, type = "text", value, onChange, placeholder, disabled, dark }) {
  return (
    <div>
      ...
      <input
        ...
        className={`w-full px-4 py-3 rounded-xl text-sm border outline-none transition-all ${
          disabled
            ? dark ? "bg-slate-800/50 border-slate-700 text-slate-500 cursor-not-allowed" : "bg-slate-50 border-slate-200 text-slate-400 cursor-not-allowed"
            : dark ? "bg-slate-800 border-slate-700 text-slate-100 placeholder-slate-500 focus:border-sky-500 focus:ring-2 focus:ring-sky-500/20"
...
function SaveBtn({ onClick, loading, saved, dark }) {
  if (saved) return (
    <div className={`px-6 py-2.5 rounded-xl text-sm font-semibold text-center border ${
      dark ? "bg-green-950/60 border-green-800/40 text-green-400" : "bg-green-50 border-green-200 text-green-700"
    }`}>✓ Desat!</div>
  );
  return (
    <button onClick={onClick} disabled={loading}
      className="px-6 py-2.5 rounded-xl text-sm font-semibold text-white disabled:opacity-50 disabled:cursor-not-allowed hover:-translate-y-px transition-all"
...
// ─── Robot Section ────────────────────────────────────────────────────────────
// Fragment per substituir RobotSection a src/app/dashboard/settings/page.js

function RobotSection({ dark }) {
